...
## Planejar o pedido — comparação com a coleção anterior + mesmo carrinho do Catálogo (ago/2026)

`Planning.tsx` (rota `/planejamento`, pontos de entrada: nav "Planejar o pedido", Radar → "Planejar pedido", Ficha de Decisão → "Planejar pedido") deixou de ser uma tela solta com barra de mix por % — o cliente achou confuso porque a barra não tinha ligação nenhuma com a lista de itens embaixo dela, e o modal "Ajustar mix" mexia num número que não afetava nada.

**Segunda rodada de revisão (mesmo mês):** mesmo depois do redesign acima, o usuário questionou o motivo de existir — "por que não comprar direto no Catálogo?" Investigando, achamos a causa raiz: o botão "Enviar plano ao carrinho" só fazia `navigate('/carrinhos')`, **não enviava nada de verdade** — as quantidades ajustadas na tela viviam num estado local (`qtyByCollection`) desconectado do `cartItems` real (o mesmo que o Catálogo e o `OrderDrawer` usam). A tela prometia uma coisa e não entregava, e por baixo disso mais nada distinguia essa tela de "uma segunda forma de mexer em número" igual ao Catálogo. Correção:

- **Renomeada** de "Planejar a coleção" para **"Planejar o pedido"** (nav, breadcrumb, título, `document`) — "coleção" era um conceito solto; "pedido" é o objeto real do sistema (o mesmo que `OrderDrawer` já chama de "Seu pedido").
- **Estado unificado**: `ProductLineCard` não recebe mais `qty`/`onChangeQty` na prop `planning` (só `{ prevQty, isGap }`) — o "Planejando agora" de cada card lê/escreve direto em `cartItems` (zustand) via `addToCart`/`removeFromCart`, exatamente como o botão "Adicionar ao carrinho" do Catálogo já fazia. Mexer no stepper aqui **é** editar o pedido real — sem estado paralelo, sem passo de "enviar".
- **Botão "Enviar plano ao carrinho" virou "Ir para o carrinho"** — mesma rota (`/carrinhos`) e mesmo rótulo do `OrderDrawer`, porque não tem mais nada pra "enviar" (já está sincronizado quando o lojista chega no botão).
- **Diferencial real que sobrou pra essa tela** (o que só ela faz, que o Catálogo não faz): mostrar **todas** as 7 linhas lado a lado, inclusive as que o lojista ainda não tocou nessa sessão de compra, com a tag `.pw-gaptag` "Lacuna no seu pedido" (linha com `cartItems` zerado em todas as cores) — ajuda a notar uma linha que sempre compra e esqueceu dessa vez. `isGap` é calculado em `Planning.tsx` (`lineCartQty()`, soma o `cartItems` de todas as cores da linha, não só a cor selecionada no momento).
- **Resumo no topo** (`.plan-summarybar`) e **sidebar** agora usam `cartSummary(cartItems)` (o mesmo helper do `OrderDrawer`) pro total "agora" — números idênticos aos que aparecem no drawer, por definição (mesmo dado).
- **Comparação por linha** (`prevQtyOf`, delta, aviso de queda >15%) continua vindo de `previousCollectionQty`/`previousCollectionStatus`, sem mudança — só o lado "agora" que deixou de ser um estado próprio.
- **Gap que continua em aberto** (não resolvido por essa mudança, é maior/separado): `cartItems` ainda não vira um `Pedido` de verdade em `carrinhos` (dado mock usado em Meus Carrinhos/Pagamento) quando o lojista clica "Ir para o carrinho" — ver gap já documentado na seção "Carrinho → Pedido" acima.

...
- **`commitCartToCarrinho()`** (`lib/store.ts`) — chamado pelo botão "Ir para o carrinho" do `OrderDrawer`: converte o `cartItems` atual num `Pedido` de verdade (`items` via `cartSummary`, `grade` derivada do miolo de `product.suggestedSizes`, `marginPct` calculado de PDV vs. fábrica, condição `'30'`/15 dias como padrão) e o anexa a um carrinho, depois **limpa o `cartItems`** — o drawer volta a representar "o próximo pedido", vazio, pronto pra montar de novo. Retorna o id do carrinho de destino (ou `null` se o drawer estava vazio — nesse caso o botão fica desabilitado).
- **`activeCarrinhoId`** (zustand) decide em qual carrinho o próximo commit entra — `null` sempre cria um carrinho novo (nome sequencial `Carrinho N`). Quem seta esse estado, replicando os dois botões que já existiam nas telas: **"+ Novo carrinho"** (em `MeusCarrinhos` e na aba de `CarrinhoDetail`) limpa pra `null`; **"Criar novo pedido neste carrinho"**, **"+ Adicionar N itens"** e **"Continuar comprando"** dentro de um `CarrinhoDetail` específico (`shopMoreForThisCarrinho()`) apontam pra aquele carrinho, então o próximo pedido do drawer volta pro mesmo lugar em vez de criar um carrinho novo à toa.

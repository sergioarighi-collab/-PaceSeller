...
- **`WizardStep2.tsx`**: "Conectar meu sistema de vendas" (integração de ERP) não tinha ação — mostra `Toast` "em breve", reforçando o "ou preencha manualmente" que já existia ao lado. **Renomeado pra "Conectar meu estoque via Pace Stock" (ago/2026)** — ver seção "Pace Stock" abaixo pro racional completo.

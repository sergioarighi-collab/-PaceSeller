...
**Discurso de valor explícito na tela (pedido do usuário, ago/2026):** o card não é só "conectar ou preencher na mão" de forma neutra — precisa deixar claro o *custo* de não conectar. Adicionado abaixo do divisor "ou preencha manualmente": "Sem conectar, o Radar usa só o que você preencher aqui — recomendações com dado real de vendas chegam depois de alguns meses, quando seu histórico se consolidar no sistema." A lógica de negócio real: o Radar precisa de volume de vendas registrado no Pace Seller pra gerar inteligência confiável; conectar via Pace Stock pula essa espera porque importa histórico já existente em vez de esperar ele se acumular do zero.

...
- **Gap que continua em aberto** (não resolvido por essa mudança, é maior/separado): `cartItems` ainda não vira um `Pedido` de verdade em `carrinhos` (dado mock usado em Meus Carrinhos/Pagamento) — ver gap já documentado na seção "Carrinho → Pedido" acima. É por isso que a comparação em `CarrinhoDetail.tsx` lê os itens estáticos de `cart.pedidos`, não o `cartItems` ao vivo do zustand.

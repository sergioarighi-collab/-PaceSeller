...
**Cor do `.pw-addbtn` — cinza/preto em vez de branco/verde (ago/2026, pedido do usuário).** Estado padrão ("Adicionar ao carrinho") virou fundo `--surface-2` cinza (antes `--surface` branco com borda) — mais claramente "não ativado". Estado `.in-cart` ("✓ No carrinho") virou preto sólido (`--black`/`--on-black`, antes verde `--positive-dim`) — mesma linguagem de "selecionado = preto" já usada em `.sizechip.selected` e `.chip.selected`, em vez de tratar como confirmação de sucesso (verde). Como a classe é compartilhada, a mudança vale em qualquer lugar que reusa `.pw-addbtn`: card do Catálogo (`Catalog.tsx`), `ProductLineCard.tsx`, e o botão "Adicionar combo ao carrinho" (`toggleCombo`).

**Hover no card de produto — leve elevação (ago/2026, pedido do usuário).** `.pcard-web` (o card do produto, usado em `ProductLineCard.tsx` e nos cards de contexto em `Catalog.tsx`) ganhou `transition:transform,box-shadow` + regra `:hover` — `translateY(-3px)` e uma sombra suave. Efeito puramente decorativo/de feedback (o card em si não é clicável como um todo — a imagem e o nome já navegam pra Ficha de Decisão, o hover só reforça "isso aqui é um produto interativo"). Sem `prefers-reduced-motion` — nenhum outro lugar do app trata isso ainda, mantido consistente com o padrão atual do código.
...
- **Dados de perfil do onboarding viram input de inteligência, não só um formulário (ago/2026).** Hoje `diferenciais`/`canaisVenda` (`WizardStep2.tsx`) e segmento/porte/público-alvo (`WizardStep1.tsx`) são só `useState` local — preenche, mostra na tela, e não vai pra lugar nenhum: nem persiste no store, nem alimenta nenhuma recomendação. O usuário sinalizou a intenção real: esses dados precisam (1) ser armazenados de verdade em algum lugar (hoje não existe backend nem persistência entre reloads — nem no `zustand`, que também não é isso) e (2) alimentar a lógica de recomendação — ajudar a identificar "a melhor compra pro perfil daquele lojista", não só descrever a loja. **Decisão explícita do usuário: pensar em como estruturar isso é trabalho pra depois** — não é pra desenhar schema de persistência nem lógica de recomendação por perfil agora, só registrar que o formulário de onboarding não é decorativo, é a semente de um cruzamento perfil↔catálogo que ainda não existe. Não confundir com a categoria de cliente 3/6/9 (ver "Fora de escopo" acima): aquela é uma classificação atribuída pelo representante que controla *quais linhas de produto aparecem* no Catálogo (controle de acesso); isto aqui seria o inverso — um perfil que o próprio lojista descreve, usado pra *priorizar/recomendar* dentro do que ele já pode ver, mais parecido com o "Mix ideal pro seu perfil" que já aparece (com número fixo, sem lógica por trás) no `OrderDrawer.tsx`.
...
## Hierarquia visual do onboarding (ago/2026)

Feedback do usuário: a diagramação do onboarding estava boa, mas faltava peso/contraste entre elementos — a régua de etapas mal diferenciava "atual" de "pendente", os blocos de conteúdo do `WizardStep2` flutuavam soltos em sequência, e o link de pular pro Radar quase sumia. Esboçado antes como Artifact ("Hierarquia do Onboarding") pra alinhar a direção antes de mexer no código real — **decisão consciente: resolver com peso/contraste, não com cor nova**, porque o resto do app (Radar, Catálogo, Carrinho) já é deliberadamente monocromático, cor só aparece pra sinal semântico (positivo/risco/info). Introduzir uma paleta própria pro onboarding faria ele destoar do que vem logo depois.

- **`.skip-btn`** (`mockup.css`) — o link "Já configurei, ir para o Radar" (`OnboardShell.tsx`) era texto solto com uma seta; virou um pill com borda e fundo branco, mesmo peso de um `.btn-secondary` compacto. Pedido explícito do usuário pra dar destaque a esse botão — mas sem ficar preto, pra não competir com o CTA principal do fim do fluxo.
- **Régua de etapas mais forte** (`.steplist`/`.stepitem`/`.sdot`) — três mudanças juntas: (1) a linha entre etapas concluídas agora fica preta sólida em vez de cinza uniforme o tempo todo — lê como barra de progresso preenchendo, não como decoração estática; (2) o dot da etapa atual cresce de 27px pra 36px e ganha um halo (`box-shadow` com `--surface-3`) — mais peso visual sem introduzir cor; (3) etapas pendentes (nem `.done` nem `.current`) perdem o subtítulo (`.ssub2{display:none}`) e reduzem peso/tamanho do título — abre mais distância visual entre "isso é o que importa agora" e "isso vem depois".
- **Agrupamento por bloco em `WizardStep2.tsx`** — o card do Pace Stock, o divisor e o aviso de risco viraram um único bloco visual (`.data-block`, fundo `--surface-2` + padding), com um rótulo de seção acima (`.block-label`, mesmo estilo mono do `.flabel`) dizendo "Dado de vendas". Os campos de perfil (`diferenciais`/`canaisVenda`) ganharam o rótulo "Perfil da loja" (`.profile-block`) como segundo bloco. Antes, tudo isso flutuava solto em sequência sem indicar que eram duas perguntas diferentes.
- **CTA final com sombra leve** — `.onboard-form .btn-primary` ganhou `box-shadow` (mesma família do hover dos cards de produto) só dentro do onboarding, sem tocar no `.btn-primary` global usado no resto do app — sinaliza "isso fecha a etapa" sem mudar a cor (preto sólido já é o mais forte do sistema de cores).
